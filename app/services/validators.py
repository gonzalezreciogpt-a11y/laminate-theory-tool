...
def validate_request(request: LaminateRequestModel) -> list[str]:
    ...
    for index, layer in enumerate(request.layers, start=1):
        if layer.material_id not in catalog:
            raise ValueError(f"Material desconocido '{layer.material_id}' en la capa {index}.")
        material = catalog[layer.material_id]
        if material.material_category == "core":
            raise ValueError(
                f"La capa {index} usa el core '{layer.material_id}'. Las capas deben usar materiales de fibra."
            )
        theta_deg = _normalize_orientation(layer.theta_deg)
        if theta_deg not in ALLOWED_ORIENTATIONS:
            raise ValueError(
                f"La capa {index} usa una orientacion no permitida. Solo se admiten -90, -45, 0, 45 y 90 grados."
            )
        if material.material_category == "fiber":
            if material.fiber_family == "ud" and theta_deg != 0.0:
                raise ValueError(
                    f"La capa {index} usa el material UD '{layer.material_id}' con una orientacion no valida. Las fibras UD solo admiten 0 grados."
                )
